...
<table width="1000" border="0" align="center" cellpadding="5"
    ...
    <tr>
        <td valign="top">
            ...
            <%
                if (participantStatus.equals("谈判中")
                        && (identity.equals("first") && bargainMatch.getParticipantStatus().equals("查看结果")
                        || identity.equals("second") && bargainMatch.getSecomdParticipantStatus().equals("查看结果"))) {
                    String result = "";
                    if (bargainData.getAcceptStatus().intValue() == 1) {
                        result = "接受出价!";
                    } else if (bargainData.getAcceptStatus().intValue() == 3) {
                        result = "终止谈判!";
                    }
            %>

            <div class="myFont1" style="margin: 20px;">
                <%=result%><br/> 你的利润是:<%=identity.equals("first") ? bargainMatch.getSupplierProfits()
                    : bargainMatch.getRetailerProfits()%><br/>
                你的谈判对象的利润是:<%=identity.equals("first") ? bargainMatch.getRetailerProfits()
